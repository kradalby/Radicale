...
__author__ = "Kristoffer Dalby"

"""
Kavik django storage backend.
...
URL = os.path.expanduser(config.get("storage", "kavik_url"))
USER = os.path.expanduser(config.get("storage", "kavik_user"))
PASS = os.path.expanduser(config.get("storage", "kavik_pass"))
...
def open(path, mode="r"):
    ...
    print(abs_path)
    with codecs.open(abs_path, mode, config.get("encoding", "stock")) as vcf:
        yield vcf
# pylint: enable=W0622
...
#        os.remove(self._props_path)
    ...
    def text(self):
        try:
            vcf = requests.get(URL, auth=(USER, PASS))
            return vcf.text
        except IOError:
            return ""
